...
class DebugFigureBasePlot:
    ...
    def set_widgets(self, plot_ax_pos: np.ndarray) -> None:
        """
        根据输入的位置参数plot_ax_pos在图上添加绘图窗口控件
        """
        self.plot_ax_pos = plot_ax_pos
        self.plot_ax = self.fig.add_axes(self.plot_ax_pos)

    def plot_curve(self, x_axis_data: np.ndarray) -> None:
        """
        画图和绘制曲线
        """
    ...
    def add_curve(self, curve: DebugFigureCurve) -> None:
        """
        在图中添加曲线
        """
...
# TODO(wangjintao): 完成AnyPlot类
class DebugFigureAnyPlot(DebugFigureBasePlot):
    def __init__(self, name: str, fig: matplotlib.figure.Figure, data_frame: pd.DataFrame) -> None:
    ...
    def set_widgets(self, plot_ax_pos: np.ndarray, button_ax_pos: np.ndarray, check_buttons_ax_pos: np.ndarray) -> None:
        self.plot_ax_pos = plot_ax_pos
        self.plot_ax = self.fig.add_axes(self.plot_ax_pos)
...
鼠标右键按住空白处拖动横纵向移动
"""
# TODO(wangjintao): DebugFigureSubplot类基于DebugFigureBasePlot类实现
class DebugFigureSubplot:
    def __init__(self, name: str, x_axis_data: np.ndarray, cursor_info: CursorInfo = None) -> None:
        self.name = name
        self.x_axis_data = x_axis_data
        self.xticks_density = 30 # 本图横轴刻度密度
...
class DebugFigure:
    ...
    def subplot_mouse_toggle_event(self, major_subplot: DebugFigureSubplot, mouse_event: matplotlib.backend_bases.MouseEvent) -> None:
